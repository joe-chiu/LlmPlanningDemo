...
{
    private const string OpenAiChatApiBase = "{0}/v1/chat/completions";
    private const string OpenAiImageGenerationApiBase = "{0}/v1/images/generations";
    private const string OpenAiImageEditApiBase = "{0}/v1/images/edits";
    ...
    public bool TextUseLocalApi { get; set; } = false;
    public bool ImageUseLocalApi { get; set; } = false;
        ...
        int maxTokens = 256)
    {
        ...
        using HttpResponseMessage responseMessage = await this.client.PostAsync(
            this.GetTextApiEndpoint(OpenAiChatApiBase), stringContent);
        ...
        int maxTokens = 256)
    {
        ...
        using HttpResponseMessage responseMessage =
            await this.client.PostAsync(this.GetTextApiEndpoint(OpenAiChatApiBase), stringContent);
        responseMessage.EnsureSuccessStatusCode();
        ...
        string prompt, ImageResponseFormat format = ImageResponseFormat.Url)
    {
        ...
        using StringContent stringContent = this.SerializeStringContent(request);
        using HttpResponseMessage responseMessage = await this.client.PostAsync(
            this.GetImageApiEndpoint(OpenAiImageGenerationApiBase), stringContent);
        responseMessage.EnsureSuccessStatusCode();
        ...
        string prompt, string imagePath, string maskPath, ImageResponseFormat format = ImageResponseFormat.Url)
    {
        ...
        using HttpResponseMessage responseMessage = await this.client.PostAsync(
            this.GetImageApiEndpoint(OpenAiImageEditApiBase), content);
        responseMessage.EnsureSuccessStatusCode();
        ImageResponse response =
            await this.DeserializePayload<ImageResponse>(responseMessage);
        return response;
    ...
    private string GetTextApiEndpoint(string api)
    {
        string endpoint = string.Format(api, 
            this.TextUseLocalApi ? "http://localhost:5000" : "https://api.openai.com");
        return endpoint;
    }

    private string GetImageApiEndpoint(string api)
    {
        string endpoint = string.Format(api,
            this.ImageUseLocalApi ? "http://localhost:5000" : "https://api.openai.com");
        return endpoint;
    }
